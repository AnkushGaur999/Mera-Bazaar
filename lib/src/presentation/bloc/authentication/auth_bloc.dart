...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  /// Instance of [LocalStorageManager] for managing local storage.
  ...
  Future<void> _login(LoginEvent event, Emitter<AuthState> emit) async {
    emit(LoginLoading());
    final response = await userLoginUseCase.call(
      email: event.email,
      password: event.password,
    );

    if (response is DataSuccess) {
      await storageManager.setToken(response.data!.refreshToken!);

      emit(LoginSuccess(user: response.data!));
    } else {
      emit(LoginError(message: response.exception!.message));
    }
  }
